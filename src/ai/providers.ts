...
export interface AIProviderRequest { question: string; context: string; evidence: string; locale: Locale; }
...
export class CloudflareAIProvider implements AIProvider {
  ...
  async complete(request: AIProviderRequest): Promise<string> {
    const languageInstruction = request.locale === "es"
      ? "Responde en español profesional, conservando símbolos, fórmulas, nombres propios y códigos técnicos."
      : "Answer in professional English.";
    const response = await this.fetcher(`https://api.cloudflare.com/client/v4/accounts/${encodeURIComponent(this.accountId)}/ai/run/${this.model}`, { method: "POST", headers: { authorization: `Bearer ${this.token}`, "content-type": "application/json" }, signal: AbortSignal.timeout(12_000), body: JSON.stringify({ messages: [{ role: "system", content: `You are a concise quantitative-finance tutor. Use only the supplied evidence for facts and numbers. Never invent market prices, probabilities or calculations. Distinguish observed data from model output. ${languageInstruction}` }, { role: "user", content: `Page context: ${request.context}\nQuestion: ${request.question}\nAuthoritative evidence: ${request.evidence}` }] }) });
