  ...
  return <main className="strategy-lab">
    ...
    <div className="strategy-lab-body section-shell">
      <section className="strategy-taxonomy" aria-label={copy({ en: "Strategy taxonomy", es: "Taxonomía de estrategias" })}>
        ...
        <div className="strategy-purpose-tabs">{PURPOSES.map((item) => <button type="button" key={item} aria-pressed={purpose === item} onClick={() => setPurpose(item)}>{displayName(item)}</button>)}</div>
        <div className="strategy-preset-grid">{strategyPresets.filter((preset) => preset.purpose === purpose).map((preset) => <button type="button" key={preset.id} aria-pressed={presetId === preset.id} onClick={() => selectPreset(preset.id)}><strong>{displayName(preset.id)}</strong><span>{preset.legCount} {preset.legCount === 1 ? copy({ en: "leg", es: "pata" }) : copy({ en: "legs", es: "patas" })}</span></button>)}</div>
      </section>
