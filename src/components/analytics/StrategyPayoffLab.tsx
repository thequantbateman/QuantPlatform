...
export function StrategyPayoffLab() {
  ...
  const moveView = (current: StrategyView, key: string) => {
    const currentIndex = STRATEGY_VIEWS.indexOf(current);
    const nextIndex = key === "Home" ? 0 : key === "End" ? STRATEGY_VIEWS.length - 1 : key === "ArrowRight" ? (currentIndex + 1) % STRATEGY_VIEWS.length : key === "ArrowLeft" ? (currentIndex - 1 + STRATEGY_VIEWS.length) % STRATEGY_VIEWS.length : currentIndex;
    if (nextIndex === currentIndex && key !== "Home" && key !== "End") return;
    const next = STRATEGY_VIEWS[nextIndex];
    setView(next);
    requestAnimationFrame(() => document.getElementById(`strategy-tab-${next}`)?.focus());
  };
