...
const definitions: Partial<Record<LabId, LabDefinition>> = {
  "realized-volatility": {
    ...
    build: (scenario, intensity) => {
      ...
      return { x, series: [{ name: "20d rolling", values: rolling, color: "--chart-series-1" }, { name: "EWMA", values: ewma, color: "--chart-series-3" }], metrics: [["Latest rolling", percent(rolling.at(-1) ?? 0)], ["Peak", percent(Math.max(...rolling))], ["EWMA λ", scenario.secondary.toFixed(2)]] };
    },
  },
  "variance-risk-premium": {
    ...
    build: (scenario, intensity) => { const x = range(13, 0, 12); const implied = x.map((m) => scenario.primary ** 2 * (1 + 0.08 * Math.cos(m / 3))); const realized = x.map((m) => scenario.secondary ** 2 * (1 + intensity * 0.28 * Math.sin((m + 1) / 2))); const spread = implied.map((value, index) => value - realized[index]); return { x, series: [{ name: "implied variance", values: implied, color: "--chart-series-2" }, { name: "realized variance", values: realized, color: "--chart-series-3" }, { name: "variance spread", values: spread, color: "--chart-series-4" }], metrics: [["Mean VRP", (spread.reduce((sum, value) => sum + value, 0) / spread.length * 10_000).toFixed(1) + " bp²"], ["IV", percent(scenario.primary)], ["RV state", percent(scenario.secondary)]] }; },
  },
  "smile-skew": {
    ...
    build: (scenario, intensity) => { const x = range(49, 0.7, 1.3); const smile = x.map((m) => 0.2 + scenario.primary * Math.log(m) * intensity + scenario.secondary * Math.log(m) ** 2); const tangent = x.map((m) => 0.2 + scenario.primary * (m - 1) * intensity); return { x, series: [{ name: "implied smile", values: smile, color: "--chart-series-1" }, { name: "ATM tangent", values: tangent, color: "--chart-series-3" }], metrics: [["ATM", percent(0.2)], ["ATM skew", (scenario.primary * intensity).toFixed(3)], ["70% wing", percent(smile[0])]] }; },
  },
  "term-structure": {
  ...
  sabr: {
    ...
    build: (scenario, intensity) => { const x = range(49, 0.7, 1.3); const smile = x.map((m) => 0.19 + 0.12 * scenario.primary * intensity * Math.log(m) + 0.22 * scenario.secondary * Math.log(m) ** 2); const backbone = x.map((m) => 0.19 * m ** (-scenario.secondary * intensity)); return { x, series: [{ name: "SABR approximation", values: smile, color: "--chart-series-1" }, { name: "backbone", values: backbone, color: "--chart-series-3" }], metrics: [["ATM", percent(0.19)], ["ρ proxy", scenario.primary.toFixed(2)], ["ν proxy", scenario.secondary.toFixed(2)]] }; },
  },
  calibration: {
    ...
    build: (scenario, intensity) => { const x = range(31, 0, 30); const objective = x.map((i) => 0.12 * Math.exp(-scenario.primary * i / 4) + (scenario.id === "local" ? 0.013 : 0.001) + 0.004 * Math.abs(Math.sin(i)) / (i + 1)); const condition = x.map((i) => 0.015 + scenario.secondary * intensity * (1 - Math.exp(-i / 8)) * 0.08); return { x, series: [{ name: "weighted RMSE", values: objective, color: "--chart-series-1" }, { name: "condition (scaled)", values: condition, color: "--chart-series-2" }], metrics: [["Final RMSE", objective.at(-1)?.toFixed(4) ?? "—"], ["Condition state", scenario.label], ["Iterations", "30"]] }; },
  },
  "higher-order-risk": {
    ...
    build: (scenario, intensity) => { const x = range(41, -10, 10); const first = x.map((spot) => 0.52 * spot + scenario.primary * intensity * (-spot / 100)); const second = x.map((spot, index) => first[index] + 0.5 * 0.055 * spot ** 2 + scenario.secondary * spot * Math.abs(spot) * 0.04); return { x, series: [{ name: "first order", values: first, color: "--chart-series-3" }, { name: "with vanna / volga", values: second, color: "--chart-series-1" }], metrics: [["Vega / point", (scenario.primary * 0.01).toFixed(3)], ["Cross state", scenario.label], ["Max residual", Math.max(...second.map((v, i) => Math.abs(v - first[i]))).toFixed(2)]] }; },
  },
};
...
export function VolatilityConceptLab({ lesson }: { lesson: AcademyLesson }) {
  ...
  const xFormatter = definition.xLabel.includes("years") || definition.xLabel === "Simulation time" ? formatYear : definition.xLabel.includes("moneyness") ? (value: number) => value.toFixed(2) : (value: number) => value.toFixed(0);
  const yFormatter = ["realized-volatility", "smile-skew", "sabr"].includes(lab.id) ? formatPercent : definition.yLabel.includes("P&L") ? (value: number) => value.toFixed(2) : (value: number) => value.toFixed(4);
  ...
  return <div className="vol-concept-lab">
    ...
    <div className="vol-concept-chart"><LineChart x={result.x} series={result.series} xLabel={definition.xLabel} yLabel={definition.yLabel} description={`${scenario.label}: ${scenario.note}.`} xFormatter={xFormatter} yFormatter={yFormatter} height={410} /></div>
