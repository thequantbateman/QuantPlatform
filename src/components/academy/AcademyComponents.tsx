...
export function SourceReferences({ references, locale }: { references: AcademyReference[]; locale: Locale }) {
  ...
  return <div className="academy-references">{references.map((reference) => { const source = findAcademySource(reference.sourceId); return <article key={`${reference.sourceId}-${reference.locator}`}><header><span>{source ? roles[source.role] : pick(locale, { en: "source", es: "fuente" })}</span></header><h3>{reference.locator}</h3><p>{reference.note}</p><dl><div><dt>{pick(locale, { en: "Source", es: "Fuente" })}</dt><dd>{source?.name}</dd></div><div><dt>{pick(locale, { en: "Author", es: "Autor" })}</dt><dd>{source?.author}</dd></div><div><dt>Ref</dt><dd>{source?.ref}</dd></div></dl><a href={reference.url} target="_blank" rel="noreferrer">{pick(locale, { en: "OPEN ORIGINAL SOURCE", es: "ABRIR FUENTE ORIGINAL" })} ↗</a></article>; })}</div>;
}
