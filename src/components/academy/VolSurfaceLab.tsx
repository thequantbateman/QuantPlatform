...
export function VolSurfaceLab({ compact = false }: { compact?: boolean }) {
  ...
  return <div className={`academy-vol-lab ${compact ? "is-compact" : ""}`}>
    ...
    <div className="vol-lab-tabs" role="tablist" aria-label={pick(locale, { en: "Volatility surface views", es: "Vistas de la superficie de volatilidad" })}>{(["heatmap", "3d", "smile", "term"] as View[]).map((item) => <button role="tab" aria-selected={view === item} className={view === item ? "active" : ""} type="button" onClick={() => setView(item)} key={item}>{item === "term" ? pick(locale, { en: "TERM STRUCTURE", es: "ESTRUCTURA TEMPORAL" }) : item.toUpperCase()}</button>)}</div>
    <div className="vol-lab-layout"><aside className="vol-lab-controls">
      ...
      <div className="vol-scenario-picker"><span>{pick(locale, { en: "CONTROLLED SCENARIO", es: "ESCENARIO CONTROLADO" })}</span>{scenarios.map((scenario) => <button type="button" aria-pressed={params.scenario === scenario.id} className={params.scenario === scenario.id ? "active" : ""} onClick={() => selectScenario(scenario.id)} key={scenario.id}><b>{locale === "es" ? ({ Base: "Base", "Spot crash": "Caída de spot", "Vol spike": "Pico de volatilidad", "Term inversion": "Inversión temporal", "Skew steepening": "Aumento del skew", Normalization: "Normalización" } as Record<string, string>)[scenario.label] : scenario.label}</b><small>{locale === "es" ? ({ base: "Superficie educativa estática", "spot-crash": "Spot −18%; suben la vol corta y el skew bajista", "vol-spike": "Shock amplio con mayor impacto frontal", "term-inversion": "Domina la incertidumbre de corto plazo", "skew-steepening": "El ala bajista se revaloriza más", normalization: "La superficie tensionada revierte hacia la base" } as Record<string, string>)[scenario.id] : scenario.note}</small></button>)}</div>
      <div className="vol-playback"><div><button type="button" onClick={() => { if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return; setPlaying((current) => !current); }} disabled={params.scenario === "base"}>{playing ? pick(locale, { en: "PAUSE", es: "PAUSA" }) : "PLAY"}</button><output>T{Math.round(params.phase * 4)}</output></div><input aria-label={pick(locale, { en: "Scenario time", es: "Tiempo del escenario" })} type="range" min="0" max="1" step="0.01" value={params.phase} onChange={(event) => { setPlaying(false); update("phase", Number(event.target.value)); }} /><p>{locale === "es" ? "Simulación controlada únicamente." : `${currentScenario.note}. Controlled simulation only.`}</p></div>
      ...
      {view === "heatmap" && <Heatmap grid={grid} selected={selected} onSelect={selectPoint} locale={locale} />}
      {view === "3d" && <VolSurfaceCanvas grid={grid} selected={selected} onSelect={selectPoint} />}
      {view === "smile" && <div className="vol-slice-view"><header><div><span>{pick(locale, { en: "SMILE SLICE", es: "CORTE DE SONRISA" })}</span><b>{tenors.find((tenor) => Math.abs(tenor.value - maturity) < 0.002)?.label ?? `${maturity.toFixed(2)}Y`}</b></div><div className="vol-tenors">{tenors.map((tenor) => <button type="button" className={Math.abs(tenor.value - maturity) < 0.002 ? "active" : ""} onClick={() => setMaturity(tenor.value)} key={tenor.label}>{tenor.label}</button>)}</div></header><LineChart x={smile.map((point) => point.x)} series={[{ name: pick(locale, { en: "implied volatility", es: "volatilidad implícita" }), values: smile.map((point) => point.y * 100), color: "#bd7653" }]} xLabel={pick(locale, { en: "Moneyness K / S", es: "Moneticidad K / S" })} yLabel={pick(locale, { en: "Implied volatility (%)", es: "Volatilidad implícita (%)" })} height={390} /></div>}
      {view === "term" && <div className="vol-slice-view"><header><div><span>{pick(locale, { en: "TERM SLICE", es: "CORTE TEMPORAL" })}</span><b>{pick(locale, { en: "MONEYNESS", es: "MONETICIDAD" })} {moneyness.toFixed(2)}</b></div><input aria-label={pick(locale, { en: "Selected moneyness", es: "Moneticidad seleccionada" })} type="range" min="0.7" max="1.3" step="0.01" value={moneyness} onChange={(event) => setMoneyness(Number(event.target.value))} /></header><LineChart x={term.map((point) => point.x)} series={[{ name: pick(locale, { en: "implied volatility", es: "volatilidad implícita" }), values: term.map((point) => point.y * 100), color: "#bd7653" }]} xLabel={pick(locale, { en: "Maturity (years)", es: "Vencimiento (años)" })} yLabel={pick(locale, { en: "Implied volatility (%)", es: "Volatilidad implícita (%)" })} height={390} /></div>}
