...
export function VolSurfaceLab({ compact = false }: { compact?: boolean }) {
  ...
  return <div className={`academy-vol-lab ${compact ? "is-compact" : ""}`}>
    ...
    <div className="vol-lab-layout"><aside className="vol-lab-controls">
      ...
      <div className="vol-lab-readout"><div><span>{pick(locale, { en: "SELECTED NODE", es: "NODO SELECCIONADO" })}</span><b>{selected.maturity < 0.1 ? `${Math.round(selected.maturity * 365)}D` : `${selected.maturity.toFixed(2)}Y`}</b></div><div><span>STRIKE / MONEYNESS</span><b>{selected.strike.toFixed(1)} / {selected.moneyness.toFixed(2)}</b></div><div><span>{pick(locale, { en: "IMPLIED VOL", es: "VOL. IMPLÍCITA" })}</span><b>{(selected.volatility * 100).toFixed(2)}%</b></div><div><span>{pick(locale, { en: "SCENARIO SPOT", es: "SPOT DEL ESCENARIO" })}</span><b>{(selected.strike / selected.moneyness).toFixed(1)}</b></div></div>
