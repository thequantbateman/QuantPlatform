...
import { formatCurrency, formatRate, formatYear } from "@/src/components/charts/chartModel";
...
export function RatesCurveLab() {
  ...
  const result = useMemo(() => {
    ...
    if (view === "zero") series = [{ name: pick(locale, { en: "base zero", es: "cero base" }), values: denseBase, color: "--chart-series-3" }, { name: pick(locale, { en: "shocked zero", es: "cero perturbado" }), values: denseShock, color: "--chart-series-2" }];
    if (view === "forward") series = [{ name: pick(locale, { en: "base forward", es: "forward base" }), values: baseForwards, color: "--chart-series-3" }, { name: pick(locale, { en: "shocked forward", es: "forward perturbado" }), values: shockedForwards, color: "--chart-series-2" }];
    if (view === "discount") { yLabel = pick(locale, { en: "Discount factor", es: "Factor de descuento" }); series = [{ name: pick(locale, { en: "base discount", es: "descuento base" }), values: baseDiscounts, color: "--chart-series-3" }, { name: pick(locale, { en: "shocked discount", es: "descuento perturbado" }), values: shockedDiscounts, color: "--chart-series-2" }]; }
    if (view === "risk") { x = nodeTimes; yLabel = pick(locale, { en: "Currency PV / scenario", es: "VA en divisa / escenario" }); series = [{ name: "DV01", values: keyRisk, color: "--chart-series-3" }, { name: pick(locale, { en: "scenario P&L ×100", es: "P&L escenario ×100" }), values: scenarioPnl.map((value) => value * 100), color: "--chart-series-2" }]; }
  ...
  return <div className="academy-vol-lab rates-curve-lab">
    ...
    <div className="vol-lab-layout"><aside className="vol-lab-controls"><header><span>{pick(locale, { en: "CURVE STATE", es: "ESTADO DE CURVA" })}</span><button type="button" onClick={() => { setScenario("base"); setIntensity(0.7); }}>{pick(locale, { en: "RESET", es: "RESTABLECER" })}</button></header><label className="vol-control"><span><b>{pick(locale, { en: "SHOCK INTENSITY", es: "INTENSIDAD DEL SHOCK" })}</b><output>{Math.round(intensity * 100)}%</output></span><input aria-label={pick(locale, { en: "Curve shock intensity", es: "Intensidad del shock de curva" })} type="range" min="0" max="1" step="0.01" value={intensity} onChange={(event) => setIntensity(Number(event.target.value))} /></label><div className="vol-scenario-picker"><span>{pick(locale, { en: "CONTROLLED SCENARIOS", es: "ESCENARIOS CONTROLADOS" })}</span>{scenarios.map((item) => <button type="button" className={scenario === item.id ? "active" : ""} aria-pressed={scenario === item.id} onClick={() => setScenario(item.id)} key={item.id}><b>{locale === "es" ? item.es : item.en}</b><small>{locale === "es" ? item.noteEs : item.noteEn}</small></button>)}</div></aside>
      <div className="vol-lab-output"><div className="vol-lab-readout">{result.metrics.map(([label, value]) => <div key={label}><span>{label}</span><b>{value}</b></div>)}</div><div className="rates-curve-chart"><header><span>{pick(locale, { en: "LINKED CURVE VIEW", es: "VISTA DE CURVA CONECTADA" })}</span><b>{view.toUpperCase()} · {scenario.toUpperCase()}</b></header><LineChart x={result.x} series={result.series} xLabel={result.xLabel} yLabel={result.yLabel} description={pick(locale, { en: `${view} curve diagnostic under the ${scenario} scenario.`, es: `Diagnóstico de curva ${view} bajo el escenario ${scenario}.` })} xFormatter={formatYear} yFormatter={view === "zero" || view === "forward" ? (value) => formatRate(value, 3) : view === "discount" ? (value) => value.toFixed(6) : (value) => formatCurrency(value, "USD", 0)} showTable height={460} /></div><footer className="rates-curve-note"><b>{pick(locale, { en: "READ THE DIAGNOSTIC", es: "LEE EL DIAGNÓSTICO" })}</b><p>{pick(locale, { en: "A curve shock updates zero rates, discount factors, interval forwards and quote-space risk from one state. Use pointer, touch or keyboard for exact values.", es: "Un shock actualiza tipos cero, factores de descuento, forwards de tramo y riesgo en cotizaciones desde un único estado. Usa puntero, toque o teclado para valores exactos." })}</p></footer></div>
    </div>
  </div>;
}
