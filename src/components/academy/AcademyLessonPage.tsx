...
export function AcademyLessonPage({ lesson }: { lesson: AcademyLesson }) {
  ...
  return <article className="academy-lesson section-shell">
    ...
    <div className="academy-lesson-layout"><aside className="academy-lesson-rail"><span>{pick(locale, { en: "PREREQUISITES", es: "PRERREQUISITOS" })}</span>{localized.prerequisites.map((item) => <p key={item}>{item}</p>)}<span>TAGS</span><div>{localized.tags.map((tag) => <b key={tag}>{tag}</b>)}</div><span>{pick(locale, { en: "REVIEWED", es: "REVISADO" })}</span><p>{localized.lastReviewed}</p></aside><main>
      ...
      <LessonSection index="03" label={pick(locale, { en: "MATHEMATICS", es: "MATEMÁTICAS" })} title={pick(locale, { en: "Notation, units and exact claims.", es: "Notación, unidades y afirmaciones exactas." })} id="mathematics"><div className="formula-stack">{localized.mathematics.formulas.map((formula, formulaIndex) => {
      ...
      <LessonSection index="05" label={pick(locale, { en: "MODEL / PRICING", es: "MODELO / VALORACIÓN" })} title={pick(locale, { en: "Fit, compute, then challenge the assumptions.", es: "Ajusta, calcula y después cuestiona los supuestos." })} id="pricing"><div className="pricing-grid"><article><span>{pick(locale, { en: "METHOD", es: "MÉTODO" })}</span><p>{localized.pricing.method}</p></article><article><span>{pick(locale, { en: "CALIBRATION", es: "CALIBRACIÓN" })}</span><p>{localized.pricing.calibration}</p></article></div>{hasAuthoredModelComparison && <ModelComparison />}{localized.implementation.quantLib && <details className="quantlib-note"><summary>{pick(locale, { en: "Implementation with current QuantLib", es: "Implementación con QuantLib actual" })}</summary><p>{localized.implementation.quantLib}</p><small>{pick(locale, { en: "API authority: upstream QuantLib reference pinned in the source registry.", es: "Autoridad de API: referencia upstream de QuantLib fijada en el registro de fuentes." })}</small></details>}</LessonSection>
      <LessonDisclosure index="06" label={pick(locale, { en: "PYTHON IMPLEMENTATION", es: "IMPLEMENTACIÓN PYTHON" })} title={pick(locale, { en: "Open the implementation and checks.", es: "Abre la implementación y sus comprobaciones." })} id="python"><div className="implementation-contract"><span>{pick(locale, { en: "ARCHITECTURE", es: "ARQUITECTURA" })}</span><ul>{localized.implementation.architecture.map((item) => <li key={item}>{item}</li>)}</ul></div><PythonLab {...localized.implementation.pythonLab} /></LessonDisclosure>
      <LessonSection index="07" label={pick(locale, { en: "INTERACTIVE LAB", es: "LABORATORIO INTERACTIVO" })} title={hasSurface ? pick(locale, { en: "One surface, four linked views.", es: "Una superficie, cuatro vistas conectadas." }) : hasCurveWorkbench ? pick(locale, { en: "One curve, four linked diagnostics.", es: "Una curva, cuatro diagnósticos conectados." }) : pick(locale, { en: "Move the state. Challenge the equation.", es: "Mueve el estado y cuestiona la ecuación." })} id="interactive" wide>{hasSurface ? <LazyVolSurfaceLab /> : hasCurveWorkbench ? <RatesCurveLab /> : localized.id === "vol-implied" ? <QuantVisual title={pick(locale, { en: "Implied-volatility inversion", es: "Inversión de volatilidad implícita" })} eyebrow={pick(locale, { en: "NUMERICAL FLOW", es: "FLUJO NUMÉRICO" })} annotation={pick(locale, { en: "The deterministic quant lab solves the same bracketed inverse with residual diagnostics.", es: "El laboratorio determinista resuelve la misma inversión acotada y muestra el residuo." })} caption={pick(locale, { en: "The implementation remains shared with the platform’s typed pricing engine.", es: "La implementación se comparte con el motor de valoración tipado de la plataforma." })}><a className="academy-open-lab" href="/lab?lab=vanilla">{pick(locale, { en: "OPEN IMPLIED-VOL SOLVER", es: "ABRIR SOLVER DE VOL IMPLÍCITA" })} →</a></QuantVisual> : isAdvancedTrack ? <AdvancedConceptLab lesson={localized} /> : isRates ? <RatesConceptLab lesson={localized} /> : <VolatilityConceptLab lesson={localized} />}</LessonSection>
      <LessonSection index="08" label="FRONT OFFICE" title={pick(locale, { en: "Where the model meets the book.", es: "Donde el modelo se encuentra con el libro." })} id="desk" wide><OnTheDesk section={localized.frontOffice} /></LessonSection>
      <LessonDisclosure index="09" label={pick(locale, { en: "MACRO CONNECTION", es: "CONEXIÓN MACRO" })} title={pick(locale, { en: "Open the transmission channel.", es: "Abre el canal de transmisión." })} id="macro">{localized.macroConnections.map((connection) => <MacroFlow connection={connection} key={connection.title} />)}</LessonDisclosure>
      <LessonDisclosure index="10" label={pick(locale, { en: "COMMON PITFALLS", es: "ERRORES FRECUENTES" })} title={pick(locale, { en: "Open the failure checklist.", es: "Abre la lista de fallos." })} id="pitfalls"><div className="pitfall-list">{localized.pitfalls.map((item, index) => <div key={item}><span>{String(index + 1).padStart(2, "0")}</span><p>{item}</p></div>)}</div></LessonDisclosure>
      <LessonDisclosure index="11" label={pick(locale, { en: "SOURCES / FURTHER READING", es: "FUENTES / LECTURAS" })} title={pick(locale, { en: "Open sources and continue the track.", es: "Abre las fuentes y continúa el itinerario." })} id="sources"><SourceReferences references={localized.references} />{related.length > 0 && <div className="academy-related"><span>{pick(locale, { en: "CONTINUE THE TRACK", es: "CONTINUAR EL ITINERARIO" })}</span>{related.map((item) => <a href={`/learn/${item.domain}/${item.slug}`} key={item.id}><b>{item.title}</b><p>{item.subtitle}</p><i>→</i></a>)}</div>}</LessonDisclosure>
    </main></div>
  </article>;
}
