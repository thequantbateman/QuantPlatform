...
export function AcademyLessonPage({ lesson }: { lesson: AcademyLesson }) {
  ...
  const trackTitle = isRates ? pick(locale, { en: "Rates & curves", es: "Tipos y curvas" }) : pick(locale, { en: "Volatility", es: "Volatilidad" });
  ...
  return <article className="academy-lesson section-shell">
    <header className="academy-lesson-hero">
      <div className="breadcrumb"><a href="/learn">Academy</a><span>/</span><a href={`/learn#track-${localized.domain}`}>{trackTitle}</a><span>/</span><b>{localized.title}</b></div>
