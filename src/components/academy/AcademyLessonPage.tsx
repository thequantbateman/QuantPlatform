...
export function AcademyLessonPage({ lesson }: { lesson: AcademyLesson }) {
  ...
  return <article className="academy-lesson section-shell">
    ...
    <div className="academy-lesson-layout"><aside className="academy-lesson-rail"><span>{pick(locale, { en: "PREREQUISITES", es: "PRERREQUISITOS" })}</span>{localized.prerequisites.map((item) => <p key={item}>{item}</p>)}<span>{pick(locale, { en: "TAGS", es: "ETIQUETAS" })}</span><div>{localized.tags.map((tag) => <b key={tag}>{tag}</b>)}</div><span>{pick(locale, { en: "REVIEWED", es: "REVISADO" })}</span><p>{localized.lastReviewed}</p></aside><main>
      ...
      <LessonSection index="08" label="FRONT OFFICE" title={pick(locale, { en: "Where the model meets the book.", es: "Donde el modelo se encuentra con el libro." })} id="desk" wide><OnTheDesk section={localized.frontOffice} locale={locale} /></LessonSection>
      <LessonDisclosure index="09" label={pick(locale, { en: "MACRO CONNECTION", es: "CONEXIÓN MACRO" })} title={pick(locale, { en: "Open the transmission channel.", es: "Abre el canal de transmisión." })} id="macro">{localized.macroConnections.map((connection) => <MacroFlow connection={connection} locale={locale} key={connection.title} />)}</LessonDisclosure>
