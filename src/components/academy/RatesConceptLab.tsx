...
const definitions: Partial<Record<LabId, Definition>> = {
  ...
  "curve-interpolation": {
    ...
    build: (scenario, intensity, locale) => { const x = range(61, 0.1, 30); const logLinear = x.map((t) => scenario.primary + scenario.secondary * intensity * (Math.floor(t / 5) % 2 ? 1.2 : 0.8)); const smooth = x.map((t) => scenario.primary + scenario.secondary * intensity * (1 - Math.exp(-t / 8)) + (scenario.id === "kink" ? 0.008 * Math.exp(-1 * (((t - 5) / 1.2) ** 2)) : 0)); return { x, series: [{ name: pick(locale, c("log-linear discount", "descuento log-lineal")), values: logLinear, color: "#f59e0b" }, { name: pick(locale, c("monotone smooth", "monótona suave")), values: smooth, color: "#22d3ee" }], metrics: [[c("Max forward gap", "Brecha forward máxima"), basisPoints(Math.max(...logLinear.map((value, i) => Math.abs(value - smooth[i]))))], [c("Minimum forward", "Forward mínimo"), percent(Math.min(...smooth))], [c("Boundary state", "Estado de frontera"), pick(locale, scenario.label)]] }; },
  },
  "multi-curve": {
    ...
    build: (scenario, intensity, locale) => { const x = range(41, 0.25, 20); const ois = x.map((t) => 0.028 + 0.014 * (1 - Math.exp(-t / 6))); const basis = x.map((t) => scenario.primary * intensity + scenario.secondary * Math.exp(-t / 3)); const projection = ois.map((rate, index) => rate + basis[index]); return { x, series: [{ name: "OIS", values: ois, color: "#22d3ee" }, { name: pick(locale, c("term projection", "proyección term")), values: projection, color: "#f59e0b" }, { name: pick(locale, c("basis", "base")), values: basis, color: "#fb7185" }], metrics: [[c("1Y basis", "Base 1A"), basisPoints(basis[2])], [c("5Y basis", "Base 5A"), basisPoints(basis[10])], [c("10Y projection", "Proyección 10A"), percent(projection[20])]] }; },
  },
  "curve-risk": {
...
export function RatesConceptLab({ lesson }: { lesson: AcademyLesson }) {
  ...
  return <div className="vol-concept-lab rates-concept-lab">
    ...
    <div className="vol-concept-chart"><LineChart x={result.x} series={result.series} xLabel={pick(locale, definition.xLabel)} yLabel={pick(locale, definition.yLabel)} height={410} /></div>
