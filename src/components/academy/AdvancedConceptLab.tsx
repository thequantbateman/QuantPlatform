...
function GreeksLab({ lesson }: { lesson: AcademyLesson }) {
  ...
  const yAxisLabel = isHedge
    ? pick(locale, c("P&L (currency units)", "P&L (unidades monetarias)"))
    : id === "higher-order-greeks"
      ? pick(locale, c("Gamma (per spot unit²)", "Gamma (por unidad spot²)"))
      : pick(locale, c("Delta (per 1 spot unit)", "Delta (por 1 unidad spot)"));
