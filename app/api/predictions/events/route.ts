...
export async function GET(request: Request) {
  ...
  try { const discovery = await discoverPredictionEvents({ limit, offset, query, tagId, order }); const persistence = await persistPredictionDiscovery(discovery.events).catch(() => ({ persistent: false, rows: 0 })); const response = url.searchParams.get("include") === "full" ? discovery : compactPredictionDiscovery(discovery); return Response.json({ ...response, persistence }, { headers: { "cache-control": query ? "no-store" : "public, max-age=45, stale-while-revalidate=180" } }); }
