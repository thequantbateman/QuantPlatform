...
export async function GET(request: Request) {
  ...
  try {
    ...
    const persistence = await persistPredictionDiscovery(snapshot.events).catch((error: unknown) => ({ persistent: false, rows: 0, error: error instanceof Error ? error.message : "Persistence unavailable" }));
