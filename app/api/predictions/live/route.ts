...
export async function GET(request: Request) {
  ...
  const url = new URL(request.url); const tokenIds = (url.searchParams.get("tokens") ?? "").split(",").filter((value) => /^\d{20,90}$/.test(value)).slice(0, 30); const marketIds = (url.searchParams.get("markets") ?? "").split(",").map((value) => value.slice(0, 180)).slice(0, tokenIds.length);
  ...
  const connect = () => {
    ...
    socket.addEventListener("message", (event) => { if (event.data === "PONG") return; try { const patches = state.apply(JSON.parse(String(event.data)) as unknown); for (const patch of patches) { emit(patch); if (patch.type === "quote" || patch.type === "trade" || patch.type === "book") { buffer.push(patch); if (buffer.length > 1_000) buffer.splice(0, buffer.length - 1_000); } } } catch { /* Malformed provider messages are ignored, never surfaced as data. */ } });
