...
export async function GET(request: Request) {
  ...
  const connect = () => {
    ...
    socket.addEventListener("open", () => { reconnectAttempt = 0; socket?.send(JSON.stringify({ assets_ids: tokenIds, type: "market", custom_feature_enabled: true })); emit({ type: "status", status: "LIVE", observedAt: Date.now() }); if (heartbeat) clearInterval(heartbeat); heartbeat = setInterval(() => { if (socket?.readyState === WebSocket.OPEN) socket.send("PING"); }, 10_000); });
